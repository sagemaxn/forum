...
import { useUserPostsQuery, UserPostsDocument } from "../../../generated/graphql";
  ...
  if(data){
      console.log(data)
        return (
          ...
          {data.userPosts.data.map(post => <Post content={post.content} user={post.username} createdAt={post.createdAt} avatar={post.avatar} key={post._id} loggedUser={decoded.user} postID={post._id}></Post>)}
          {parseInt(page) * 5 < data.userPosts.total && <Link href={`/user/${user}/${parseInt(page) + 1}`}>Next Page</Link>}
